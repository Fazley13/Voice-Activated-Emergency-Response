...
public class UserRegisterController {
    ...
    private void handleRegister() {
        ...
        System.out.println("=== USER REGISTRATION (FIXED) ===");
        ...
        showStatus("Creating account, please wait...", "info");

        new Thread(() -> {
            try {
                boolean success = createUserAccount(username, email, password);
                
                Platform.runLater(() -> {
                    if (success) {
                        showStatus("✅ SUCCESS! Account created for " + username + "!", "success");
                        clearFields();
                        
                        Alert alert = new Alert(Alert.AlertType.INFORMATION);
                        alert.setTitle("Registration Successful");
                        alert.setHeaderText("User Account Created!");
    ...
    private boolean createUserAccount(String username, String email, String password) {
        try {
            ...
            // Insert new user - USING EXACT SQL STRUCTURE
            System.out.println("💾 Inserting new user with password_hash column...");
            String insertSql = "INSERT INTO users (username, email, password_hash, status) VALUES (?, ?, ?, ?)";
            PreparedStatement insertStmt = conn.prepareStatement(insertSql);
            insertStmt.setString(1, username);
            insertStmt.setString(2, email);
            insertStmt.setString(3, password); // For now using plain password, you can hash it later
            insertStmt.setString(4, "active");
            
            System.out.println("🔧 Executing SQL: " + insertSql);
            System.out.println("📝 Parameters: [" + username + ", " + email + ", [password], active]");
            ...
            if (rowsAffected > 0) {
                ...
                String verifySql = "SELECT id, username, email, status FROM users WHERE email = ?";
                ...
                if (verifyRs.next()) {
                    ...
                    System.err.println("❌ User not found after insertion!");
                }
                verifyRs.close();
                verifyStmt.close();
            } else {
